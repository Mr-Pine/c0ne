...
class X86CodeGenerator(irGraphs: List<IrGraph>) {
    ...
    class AbstractCodegen(irGraph: IrGraph, schedule: Schedule) {
        ...
        inner class AbstractCodegenVisitor(
            ...
            fun visitIdiv(node: BinaryOperationNode, targetRegister: RealRegister) {
                ...
                instructionList.add(Mov(RealRegister.RAX, left))
                instructionList.add(Cdq())
                instructionList.add(Idiv(rightRegMem))
                instructionList.add(Mov(target, targetRegister))
            }

            override fun visit(node: DivNode) {
                visitIdiv(node, RealRegister.RAX)
            }
            ...
            override fun visit(node: ModNode) {
                visitIdiv(node, RealRegister.RDX)
            }
            ...
            override fun visit(node: ReturnNode) {
                ...
                instructionList.add(Mov(RealRegister.RAX, value))
